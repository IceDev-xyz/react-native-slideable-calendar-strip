      ...
      <View style={styles.itemContainer}>
        ...
        >
          <View style={[
            ...
            <Text style={[
              styles.itemDateText,
              {color: highlight ? hightlightTextColor : normalTextColor, fontWeight: (today == solar) ? 'bold': 'normal'}
            ]}>{solar}</Text>
            {showLunar &&
              <Text style={[
            ...
            {marked &&
              <View style={[
                ...
                //{backgroundColor: highlight ? 'white' : marked.color}
...
class CalendarStrip extends Component {
  ...
  _renderHeader = () => {
    const { selectedDate, onPressGoToday, isChinese, isSpanish, showWeekNumber } = this.props;
  ...
  _stringToDate = (dateString) => {
    // '2018-01-01' => Date
    const dateArr = dateString.date.split('-');
    const [y, m, d] = dateArr.map(ds => parseInt(ds, 10));
    // CAVEAT: Jan is 0
    let obj = {date: new Date(y, m-1, d), color: dateString.color}
    return obj;
  }
  ...
  render() {
    ...
    return (
      <View style={styles.container} {...this._panResponder.panHandlers}>
        <Weeks isChinese={isChinese} isSpanish={isSpanish} weekStartsOn={weekStartsOn} />
        <FlatList
          ...
          renderItem={({item}) => 
            <DateItem
              ...
              marked={marked.find(d => isSameDay(d.date, item))}
            />}
        />
      </View>
    );
